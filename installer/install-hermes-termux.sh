...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        # === DROIDRUNNER PATCH: resync Termux bootstrap once, before any pkg install ===
        # A fresh F-Droid Termux APK ships an older usr/ snapshot than the live
        # repo. The first `pkg install` of anything pulls current-repo libs
        # (libssl, etc.) with SONAMEs the rest of the bootstrap's binaries can't
        # load — e.g. curl crashes with `cannot locate symbol SSL_set_alpn_protos`.
        # --force-confold keeps existing config files (e.g. openssl.cnf) so the
        # upgrade doesn't pause on a dpkg conffile prompt.
        if [ -z "${DROIDRUNNER_BOOTSTRAP_RESYNCED:-}" ]; then
            log_info "Resyncing Termux bootstrap (one-time, ~1 min)..."
            pkg upgrade -y \
                -o Dpkg::Options::="--force-confold" \
                -o Dpkg::Options::="--force-confdef" >/dev/null 2>&1 || true
            export DROIDRUNNER_BOOTSTRAP_RESYNCED=1
        fi
        # === END PATCH ===
...
install_system_packages() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        # === DROIDRUNNER PATCH: add python-psutil to system package list ===
        # psutil's upstream setup.py runs `sys.exit("platform android is not
        # supported")` before any compile step — no toolchain can rescue it.
        # Termux ships a patched fork as `python-psutil`; we install it here
        # and symlink it into the venv inside install_deps() below.
        local termux_pkgs=(clang rust make pkg-config libffi openssl python-psutil)
        # === END PATCH ===
...
setup_venv() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        # === DROIDRUNNER PATCH: preserve existing venv (don't wipe Rust cache) ===
        # Upstream wipes the venv on every run unless --no-venv is passed, which
        # makes a recoverable failure re-trigger the ~20-minute jiter Rust
        # build. We reuse an existing venv instead.
        if [ -d "venv" ]; then
            log_info "Virtual environment already exists, reusing (preserves Rust compile cache)."
        else
...
install_deps() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        # === DROIDRUNNER PATCH: symlink Termux psutil + point maturin at native Rust ===
        # The python-psutil package installed in install_system_packages() lives
        # in Termux's site-packages, not the venv's. Symlink the package AND
        # its dist-info into the venv so pip's resolver treats the dep as
        # already satisfied (and skips the source build that sys.exit()s on
        # Android). CARGO/RUSTC env vars stop maturin from invoking rustup,
        # which has no aarch64-unknown-linux-android channel — Termux's
        # native Rust toolchain works once these are exported.
        if [ "$USE_VENV" = true ]; then
            local _drr_pyver
            _drr_pyver="$("$PIP_PYTHON" -c 'import sys; print(f"{sys.version_info.major}.{sys.version_info.minor}")')"
            local _drr_termux_sp="${PREFIX:-/data/data/com.termux/files/usr}/lib/python${_drr_pyver}/site-packages"
            local _drr_venv_sp="$INSTALL_DIR/venv/lib/python${_drr_pyver}/site-packages"
            if [ -d "$_drr_termux_sp/psutil" ]; then
                ln -sfn "$_drr_termux_sp/psutil" "$_drr_venv_sp/psutil"
                local _drr_di
                for _drr_di in "$_drr_termux_sp"/psutil-*.dist-info; do
                    [ -e "$_drr_di" ] && ln -sfn "$_drr_di" "$_drr_venv_sp/$(basename "$_drr_di")"
                done
            fi
        fi
        ...
        # === DROIDRUNNER PATCH: repair entry-points + recover from PyYAML phantom ===
        # 1. pip's editable-install sometimes registers hermes-agent's metadata
        #    without writing venv/bin/hermes. --no-deps --force-reinstall
        #    regenerates the entry-point scripts cheaply (no dep rebuild).
        # 2. A "Successfully installed" pip run can list PyYAML in its output
        #    while the package is missing from site-packages. A second pip
        #    pass reconciles the divergence — no flags needed.
        if [ "$USE_VENV" = true ] && [ ! -x "$INSTALL_DIR/venv/bin/hermes" ]; then
            log_warn "venv/bin/hermes missing — repairing entry-points..."
            "$PIP_PYTHON" -m pip install -e . --no-deps --force-reinstall \
                -c constraints-termux.txt >/dev/null 2>&1 || true
        fi
        if [ "$USE_VENV" = true ] && [ -x "$INSTALL_DIR/venv/bin/hermes" ] \
                && ! "$INSTALL_DIR/venv/bin/hermes" --help >/dev/null 2>&1; then
            log_warn "Smoke test failed — re-running pip install once to reconcile..."
            "$PIP_PYTHON" -m pip install -e '.[termux]' \
                -c constraints-termux.txt >/dev/null 2>&1 || true
        fi
        # === END PATCH ===

        # === DROIDRUNNER PATCH: pull latest commits via `hermes update` ===
        # The script pins HERMES_REF in clone_repo() to a known-good tag, but
        # upstream main may be ahead of it (the prior install printed
        # "Update available: 1 commit behind — run 'hermes update'"). Run
        # `hermes update` non-interactively so the install lands at the
        # latest published commit. Stdin is /dev/null so any prompt becomes
        # EOF and defaults. Failure is non-fatal: the install still works
        # at the pinned tag if update can't complete.
        if [ "$USE_VENV" = true ] && [ -x "$INSTALL_DIR/venv/bin/hermes" ]; then
            log_info "Pulling latest hermes-agent commits via \`hermes update\`..."
            if "$INSTALL_DIR/venv/bin/hermes" update </dev/null; then
                log_success "Updated to latest."
            else
                log_warn "\`hermes update\` failed — install remains at pinned tag."
            fi
        fi
        # === END PATCH ===
